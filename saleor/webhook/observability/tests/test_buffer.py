...
@patch("saleor.webhook.observability.buffer.cache.make_key", side_effect=_make_key)
def test_observability_events_buffer_repr(_, memory_broker):
    with ObservabilityBuffer(memory_broker, EVENT_TYPE) as buffer:
        assert _make_key(CACHE_KEY + EVENT_TYPE) in repr(buffer)


@patch("saleor.webhook.observability.buffer.cache.make_key")
...
@patch(
    "saleor.webhook.observability.buffer.SimpleQueue.qsize", side_effect=ChannelError
)
...
@patch("saleor.webhook.observability.buffer.ObservabilityBuffer.put_event")
...
#     ],
# )
# @patch("saleor.webhook.observability.buffer.observability_event_delivery_attempt")
