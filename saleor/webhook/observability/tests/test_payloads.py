...
def test_serialize_gql_operation_result(
    ...
    assert payload == {
        ...
        "result": JsonTruncText(json_dumps(result), False),
    }


def test_serialize_gql_operation_result_when_no_operation_data():
    result = GQLOperationResult()
...
def test_serialize_gql_operation_result_when_truncated(
    ...
    assert payload == {
        ...
        "result": JsonTruncText('{"data": "resu', True),
    }
    assert left_bytes == 0
...
def test_serialize_gql_operation_results_when_minimal_bytes_limit(
    ...
    serialized = json_dumps(payloads)
    assert payloads == [
        {
            "name": JsonTruncText("", True),
            "operation_type": "query",
            ...
            "operations": [
                {
                    ...
                    "result": {"text": json_dumps(result_a), "truncated": False},
                },
                {
                    "name": {"text": "SecondQuery", "truncated": False},
                    "operation_type": "query",
                    "query": {"text": query_b, "truncated": False},
                    "result": {"text": json_dumps(result_b), "truncated": False},
                },
            ],
        },
    }
...
def test_generate_api_call_payload_skip_operations_whene_size_limit_too_low(
